...
public class SqliteDatabase
{
    ...
    public static extern int sqlite3_bind_parameter_index(IntPtr stmt, string paramName);
	...
	public SqliteDatabase (string dbName)
	{
		...
		pathDB = System.IO.Path.Combine(Application.dataPath, dbName);// テスト用にassetsフォルダへ
		//pathDB = Application.dataPath + "/" + dbName;// テスト用にassetsフォルダへ
		//original path
	...
	public DataTable ExecuteQuery (string query, Dictionary<string, object> param = null)
	{
        ...
        if (param != null)
        {
            foreach (var p in param)
            {
                ...
                if (index == 0)
                {
                    Debug.LogWarning($"Parameter not found in query: {p.Key}");
                    continue;
                }

                if (p.Value is int)
                {
                    sqlite3_bind_int(stmHandle, index, (int)p.Value);
                }
                else if (p.Value is float || p.Value is double)
                {
                    sqlite3_bind_double(stmHandle, index, Convert.ToDouble(p.Value));
                }
                else if (p.Value is string)
                {
                    byte[] utf8Bytes = System.Text.Encoding.UTF8.GetBytes((string)p.Value);
                    sqlite3_bind_text(stmHandle, index, utf8Bytes, utf8Bytes.Length, new IntPtr(-1));
                }
                else
                {
                    Debug.LogWarning($"Unsupported parameter type: {p.Key} = {p.Value}");
                }
            }
        }
